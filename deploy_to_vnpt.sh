...
tar --exclude='.git' --exclude='node_modules' --exclude='bin' --exclude='obj' --exclude='.env' --exclude='data_dump' --exclude='._*' -czf deploy.tar.gz .
...
expect << EOF
set timeout -1
spawn ssh -o StrictHostKeyChecking=no $VNPT_USER@$VNPT_HOST "mkdir -p /root/lichcongtac; cd /root/lichcongtac; tar -xzf /root/deploy.tar.gz; docker compose -p lichcongtac up -d --build; rm /root/deploy.tar.gz"
expect {
    "password:" {
        send "$VNPT_PASS\r"
        exp_continue
