...
export function NotificationForm({ formData, setFormData, editId, loading, error, onSubmit, onReset }) {
  ...
  return (
    <form onSubmit={onSubmit} className="mb-10 w-full">
      {error && (
        <div className="bg-red-100 text-red-700 p-2 mb-4 rounded border border-red-300">{error}</div>
      )}
      ...
      <div className="flex items-center gap-2 mb-4 font-bold text-[#4cae4c]">
        ...
        <input type="checkbox" id="isVisible" name="isVisible" checked={formData.isVisible}
          onChange={handleChange} className="w-4 h-4 cursor-pointer" />
      </div>
